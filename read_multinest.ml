...
let read_multinest_sample inp = 
  let line = input_line inp in 
  let buf = Scanf.Scanning.from_string line in 
  let params_like = read_array buf in 
    {Mcmc.value = Array.sub params_like 1 9;
     like_prior = {Mcmc.log_likelihood = params_like.(0);
                   log_prior = 0.0}}

let read_multinest_samples file = 
  let inp = open_in file in 
    ...
    let samples =
      try 
        while true do 
          samples := (read_multinest_sample inp) :: !samples 
        done;
        !samples 
      with 
